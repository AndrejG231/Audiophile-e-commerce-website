...
const applyPrefix = (sources: any) => {
  // If array - check every element and change source
  if (sources instanceof Array) {
    sources.map((item, index) => {
      // If string, change it, if not call change on nested object/array
      if (typeof item === "string") {
        // Check if string is a link for image
        if (/(^\.).+(jpg|png)$/g.test(item)) {
          sources[index] = _SERVER + item.slice(1);
        }
      } else {
        applyPrefix(item);
      }
  ...
  if (typeof sources === "object") {
    Object.entries(sources).map(([key, item]) => {
      ...
      if (typeof item === "string") {
        // Check if string is a link for image
        if (/(^\.).+(jpg|png)$/g.test(item)) {
          sources[key] = _SERVER + item.slice(1);
        }
      } else {
        applyPrefix(item);
      }
